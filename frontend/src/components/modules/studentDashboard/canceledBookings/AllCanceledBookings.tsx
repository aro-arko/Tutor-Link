...
import { Calendar, User, CreditCard, DollarSign, XCircle } from "lucide-react";
import { TBooking } from "@/types/booking";

const AllCanceledBookings = () => {
      ...
      try {
        ...
        if (res.success) {
          const canceled = res.data
            .filter((b: TBooking) => b.approvalStatus === "canceled")
            .sort(
  ...
  return (
    <div className="p-1 md:p-4 mx-auto">
      <h1 className="text-3xl font-bold mb-6 text-center">Canceled Bookings</h1>

      <div className="flex justify-center mb-6">
        <div className="inline-flex items-center space-x-2 px-3 py-2 rounded-full bg-red-100 hover:bg-red-200 text-red-700 text-sm font-medium transition">
          <Calendar className="w-4 h-4" />
          <span>Total Canceled Bookings: {bookings.length}</span>
        </div>
      </div>

      {bookings.length === 0 ? (
        <p className="text-gray-600 text-center">No canceled bookings found.</p>
      ) : (
...
export default AllCanceledBookings;
